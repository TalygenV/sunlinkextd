...
import { Camera, Upload, CheckCircle, X, Home, Zap, FileText, ArrowRight, AlertCircle, Image } from 'lucide-react';
...
const PhotoUploadPage: React.FC<PhotoUploadPageProps> = ({ onComplete, customerInfo }) => {
  ...
  const uploadCategories = [
    ...
    },
    {
      ...
      description: 'Photo of your electric meter from a distance showing location',
      icon: Zap,
      required: true
    },
    {
      ...
      description: 'Photo of your most recent utility bill showing account holder name and account number',
      icon: FileText,
      required: true
    }
  ];
  ...
  const isFormComplete = () => {
    const requiredCategories = uploadCategories.filter(cat => cat.required).map(cat => cat.id);
    const uploadedCategories = uploadedFiles.map(f => f.category);
    const hasAllRequired = requiredCategories.every(cat => uploadedCategories.includes(cat));
    return hasAllRequired && roofType && roofAge;
  };

  const handleSubmit = async () => {
    if (!isFormComplete()) {
      alert('Please complete all required fields and upload all required photos');
      return;
    }
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        {existingFile ? (
          ...
          <div
            ...
            onClick={() => fileInputRefs[category.id as keyof typeof fileInputRefs]?.current?.click()}
          >
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="tesla-card rounded-2xl w-full max-w-2xl p-8 text-center animate-fade-in">
          ...
          <h2 className="text-2xl font-light text-white mb-4">Uploading Your Photos</h2>
          <p className="text-gray-300 mb-6">Processing your home photos and information...</p>
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-light text-gray-900 mb-2">Upload Home Photos</h1>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Help us prepare your solar installation by sharing photos of your home and electrical setup. 
            These photos will be used by your installer to prepare necessary permits and plan your installation.
          </p>
        </div>

        {/* Customer Info */}
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
          <h2 className="text-lg font-medium text-gray-900 mb-4">Installation Details</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <div className="font-medium text-gray-900">{customerInfo.email}</div>
            </div>
            <div>
              <span className="text-gray-500">Address:</span>
              <div className="font-medium text-gray-900">{customerInfo.address}</div>
            </div>
          </div>
        </div>
        ...
        {/* Roof Information */}
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
          <h2 className="text-lg font-medium text-gray-900 mb-6">Roof Information</h2>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <ul className="text-sm text-blue-800 space-y-1">
                ...
                <li>• For utility bill, make sure account holder name and account number are visible</li>
                <li>• Photos should be recent (taken within the last 30 days)</li>
                <li>• All personal information on utility bills will be kept confidential</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-500">
              {uploadedFiles.length} of {uploadCategories.filter(cat => cat.required).length} required photos uploaded
            </span>
          </div>
          ...
          <div className="w-full bg-gray-200 rounded-full h-2 mb-4">
            <div 
              ...
              style={{ 
                width: `${(uploadedFiles.length / uploadCategories.filter(cat => cat.required).length) * 100}%` 
              }}
            />
          </div>

          <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
            {uploadCategories.filter(cat => cat.required).map((category) => {
        ...
        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
          ...
          <button
            onClick={onComplete}
            className="tesla-button-secondary py-4 px-8 rounded-xl font-medium"
          >
            Skip for Now
          </button>
        </div>
        ...
        <div className="mt-8 bg-gray-100 rounded-xl p-6">
          <h3 className="font-medium text-gray-900 mb-4">After You Submit</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div className="flex items-start space-x-3">
              <div className="w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center text-xs font-bold">1</div>
              <div>
                <div className="font-medium text-gray-900">Review & Analysis</div>
                <div className="text-gray-600">Our team reviews your photos within 24 hours</div>
              </div>
            </div>
            <div className="flex items-start space-x-3">
              <div className="w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center text-xs font-bold">2</div>
              <div>
                <div className="font-medium text-gray-900">Permit Preparation</div>
                <div className="text-gray-600">We prepare and submit all necessary permits</div>
              </div>
            </div>
            <div className="flex items-start space-x-3">
              <div className="w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center text-xs font-bold">3</div>
              <div>
                <div className="font-medium text-gray-900">Installer Assignment</div>
                <div className="text-gray-600">You'll be matched with a certified installer</div>
              </div>
            </div>
          </div>
        </div>
