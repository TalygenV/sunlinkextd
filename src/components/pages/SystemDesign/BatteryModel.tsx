...
interface BatteryModelProps {
  ...
  scale?: number;
  rotationY?: number;
  onRotateRequest?: ((direction: "left" | "right") => void) | null;
  modelFile?: string;
  isMobile?: boolean;
}

const BatteryModel = forwardRef<any, BatteryModelProps>(
  (
    {
      containerHeight = 500,
      style = {},
    ...
    const batteryMeshRef = useRef<THREE.Group>(null);
    ...
    useImperativeHandle(ref, () => ({
      rotateModel: (direction: "left" | "right") => {
            ...
            if (Array.isArray(mesh.material)) {
              mesh.material.forEach((mat) => {
                if (!mat.transparent) {
                  ...
                  (mat as any)._originalOpacity = mat.opacity || 1;
                }
              ...
              if (!material.transparent) {
                ...
                material._originalOpacity = material.opacity || 1;
              }
              // Reduce opacity but don't make completely invisible
            ...
            modelRef.current!.traverse((node) => {
              if ((node as any).isMesh && (node as any).material) {
                ...
                if (Array.isArray(mesh.material)) {
                  ...
                  material.opacity = material._originalOpacity || 1;
                }
              }
            });
    ...
    useEffect(() => {
          ...
          model.traverse((node) => {
            if ((node as any).isMesh && (node as any).material) {
              ...
              materials.forEach((material) => {
                ...
                (material as any).metalness = 0.9; // Keep the surface metallic
                (material as any).roughness = 0.65; // Increase roughness for a more matte look
                (material as any).envMap = envMap; // Ensure reflections still work albeit subdued
                (material as any).envMapIntensity = 0.4; // Reduce reflection intensity to avoid a plasticky shine

                if ((material as any).map) {
                  (material as any).map.anisotropy =
      ...
      (mountRef.current as any).animationState = animationState;
      ...
      (mountRef.current as any).updateModelProperties = updateModelProperties;
      ...
      (mountRef.current as any).sceneRef = sceneRef;
      ...
      return () => {
        ...
        // Dispose all lights
        Object.values(lightsRef.current).forEach((light: any) => {
    ...
    useEffect(() => {
      if (mountRef.current && (mountRef.current as any).updateModelProperties) {
        // Pass the current values to the function
        (mountRef.current as any).updateModelProperties(
          position,
          scale,
          rotationY
        );
    ...
    useEffect(() => {
      const controls = (mountRef.current as any)?.sceneRef?.controls;
      if (controls) {
...
function createColorDataURL(hexColor: string) {
