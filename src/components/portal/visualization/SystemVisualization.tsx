...
import {
  ...
  InstallationStage,
} from "../progress/InstallationProgressTracker";
        ...
        if (feature.geometry.type === "Polygon") {
          ...
          return coordinates.map(
            (ring: Array<[number, number]>) =>
              ring.map((coord: [number, number]): [number, number] => [
                coord[1],
                coord[0],
              ]) // Flip lng/lat, return [number, number]
          );
...
export const SystemVisualization: React.FC<SystemVisualizationProps> = ({
    ...
    const fetchSystemData = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          if (userData.batteryCount !== undefined)
            setBatteryCount(userData.batteryCount);
          if (userData.selectedBatteryDetails)
            setSelectedBattery(userData.selectedBatteryDetails);
          if (userData.autoPanelOffset)
            setAutoPanelOffset(userData.autoPanelOffset);

          // Convert obstructed panels array to Set
          if (
            userData.obstructedPanels &&
            Array.isArray(userData.obstructedPanels)
          ) {
          ...
          if (
            userData.manualPanelRegions &&
            Array.isArray(userData.manualPanelRegions)
          ) {
          ...
          if (userData.panels && userData.panels.length > 0) {
            ...
            userData.panels.forEach((panel: any) => {
              if (
                panel.center &&
                typeof panel.center.latitude === "number" &&
                typeof panel.center.longitude === "number"
              ) {
          ...
          if (calculatedCenter) {
            ...
            console.warn(
              "Could not determine map center from panels or userData."
            );
          ...
          if (userData.progress) {
            ...
            for (let i = 0; i < stageOrder.length; i++) {
              ...
              if (
                userData.progress[stageName] &&
                userData.progress[stageName].date
              ) {
  ...
  const calculateSystemStats = (userData: any) => {
    ...
    const currentObstructedPanels =
      userData.obstructedPanels && Array.isArray(userData.obstructedPanels)
        ? new Set(userData.obstructedPanels)
        : obstructedPanels;

    const activePanels = userData.panels
      ? userData.panels.filter((panel: any) => {
          const panelId = getPanelId(panel);
          return (
            panel.isActiveInCurrentConfig &&
            !currentObstructedPanels.has(panelId)
          ); // Use the Set for check
        })
      : [];
    ...
    const panelWattage =
      userData.panels && userData.panels[0]?.wattage
        ? userData.panels[0].wattage
        : 400; // Default to 400W if not specified
  ...
  if (error) {
    return (
      <div
        className={`p-6 bg-black/20 backdrop-blur-sm rounded-xl border border-red-500/20 ${className}`}
      >
    ...
    <div className={`${className}`}>
      ...
      >
        ...
        <div className=" rounded-xl shadow-xl">
          <h3 className="text-xl font-medium text-white mb-4 pb-4">
                      ...
                      region.solarPanelData.features.map(
                        (panel: any, index: number) => {
                          ...
                          const yearlyEnergyDcKwh =
                            panel.properties?.yearlyEnergyDcKwh || 400; // Default estimate
                          const azimuthDegrees =
                            panel.properties?.azimuthDegrees || 180; // Default south-facing
                          ...
                          return (
                            <Polygon
                              ...
                              pathOptions={{
                                color: isObstructed
                                  ? "transparent"
                                  : "#FFFFFF00", // Grey for obstructed
                                fillColor: isObstructed
                                  ? "transparent"
                                  : "#000000", // Transparent for obstructed
                              ...
                              >
                                <div className="text-xs">
                                  ...
                                  <p className="text-gray-700">
                                    Energy: {yearlyEnergyDcKwh?.toFixed(2)}{" "}
                                    kWh/year
                                  </p>
                                  <p className="text-gray-700">
